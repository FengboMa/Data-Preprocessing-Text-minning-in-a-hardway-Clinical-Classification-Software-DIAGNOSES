...
A = read_delim('https://hcup-us.ahrq.gov/toolssoftware/ccs/AppendixBSinglePR.txt', skip = 2, skip_empty_rows = TRUE)

AppA = A %>% 
  # dplyr::select(2) %>% 
  # trimws()
  mutate(across(where(is.character),str_trim)) %>% 
  mutate(last = substrRight(.$`07-22-2014`,1)) 
...
final = cbind(data_save$data,test) %>% 
  ...
  filter(value!='NA') %>% 
  filter(value!='') %>% 
  setNames(c('procedure', 'code')) %>% 
  arrange(procedure)
  
write_csv(final,'C:/Users/fengb/OneDrive/Desktop/AppB.csv')
